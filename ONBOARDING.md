...
| 2 | 简历库 | 进入页面默认只显示**卡片网格**；点「新建」或卡片进入**编辑视图**（带「返回列表」） | **拆视图**验证：列表/编辑不再挤在一页；必填项（标题/姓名/手机/邮箱）红星 + 缺失前端拦截 |
| 3 | 写简历 | 「从简历库加载」→ 选目标岗位（**快捷标签 + 自定义输入**，可敲「游戏策划」等任意岗位）→ AI 生成 | 简历↔简历库双向联动；下拉字段可直接输新值 |
